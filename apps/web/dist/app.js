...
function credentialsToCsv(rows) {
  const headers = ["Bib", "AFI UID", "Athlete", "Team", "Access Code"];
  return rowsToCsv(rows.map(row => ({
    "Bib": row.bib,
    "AFI UID": row.afiUid || row.afiUID || "",
    "Athlete": row.name,
    "Team": row.team,
...
function showTab(tabId) {
  const button = $(`.nav-tabs button[data-tab="${tabId}"]`);
  if (button?.hidden) tabId = "dashboard";
  $$(".nav-tabs button").forEach(item => item.classList.toggle("active", item.dataset.tab === tabId && !item.hidden));
  $$(".tab-panel").forEach(panel => panel.classList.toggle("active", panel.id === tabId));
}

function renderAccess() {
  const organizerOpen = state.organizerSession;
  const athleteOpen = Boolean(state.athletePortal);

  $$("[data-organizer-tab]").forEach(button => {
    button.hidden = !organizerOpen;
    if (button.hidden) button.classList.remove("active");
  });
  $$("[data-athlete-tab]").forEach(button => {
    button.hidden = !athleteOpen;
    if (button.hidden) button.classList.remove("active");
  });
  $$(".organizer-only").forEach(node => {
    node.hidden = !organizerOpen;
  });

  $("#organizerAccessStatus").textContent = organizerOpen
    ? "Organizer desk is unlocked. Management tabs are available."
    : "Organizer desk is locked. Dashboard and registration remain available.";
  $("#athleteAccessStatus").textContent = athleteOpen
    ? `${state.athletePortal.athlete.name} is logged in. Athlete side is open.`
    : "Athlete side is locked.";
  $("#organizerAccessBadge").textContent = organizerOpen ? "Unlocked" : "Locked";
  $("#organizerAccessBadge").className = `access-badge ${organizerOpen ? "unlocked" : "locked"}`;
  $("#athleteAccessBadge").textContent = athleteOpen ? "Unlocked" : "Locked";
  $("#athleteAccessBadge").className = `access-badge ${athleteOpen ? "unlocked" : "locked"}`;

  const active = $(".tab-panel.active")?.id || "dashboard";
  if (!organizerOpen && !athleteOpen && !["dashboard", "registration"].includes(active)) showTab("dashboard");
  if (!organizerOpen && active && !["dashboard", "registration", "athletePortal"].includes(active)) showTab("dashboard");
  if (!athleteOpen && active === "athletePortal") showTab("dashboard");
  const current = $(".tab-panel.active")?.id || "dashboard";
  const currentButton = $(`.nav-tabs button[data-tab="${current}"]`);
  if (!currentButton || currentButton.hidden) showTab("dashboard");
}
...
function renderMeet() {
  ...
  const sidebarMeetName = $("#sidebarMeetName");
  const sidebarMeetDate = $("#sidebarMeetDate");
  if (sidebarMeetName) sidebarMeetName.textContent = state.meet.name || "Meet setup";
  if (sidebarMeetDate) {
    sidebarMeetDate.textContent = state.organizerSession
      ? ([state.meet.venue, state.meet.date].filter(Boolean).join(" - ") || "Organizer unlocked")
...
function renderAthletes() {
  ...
  $("#athletesBody").innerHTML = athletes.map(athlete => `
    <tr>
      ...
      <td><strong>${athlete.accessCode || "Not generated"}</strong></td>
      <td><button class="link-button danger" data-delete="athletes" data-id="${athlete.id}">Delete</button></td>
    </tr>
  `).join("") || emptyRow(7, "No athletes found");
}

function renderAthletePortal() {
  const portal = state.athletePortal;
  if (!portal) {
    $("#athletePortalName").textContent = "Athlete View";
    $("#athletePortalMeta").textContent = "Login to see entries and results";
    $("#athletePortalSummary").textContent = "No athlete logged in yet.";
    $("#athletePortalBody").innerHTML = emptyRow(6, "Login to view athlete events");
    return;
  }

  const athlete = portal.athlete;
  $("#athletePortalName").textContent = athlete.name;
  $("#athletePortalMeta").textContent = `${athlete.bib} - ${athlete.category || athlete.gender || "Open"} - ${athlete.team}`;
  $("#athletePortalSummary").innerHTML = `
    <strong>${portal.meet?.name || "Athletic Meet"}</strong><br>
    ${portal.meet?.venue || ""} ${portal.meet?.date ? `- ${portal.meet.date}` : ""}<br>
    Entries: ${portal.entries.length}
  `;
  $("#athletePortalBody").innerHTML = portal.entries.map(entry => `
    <tr>
      <td><strong>${entry.event.name}</strong></td>
      <td>${entry.event.category}</td>
      <td>${entry.result ? "Result entered" : "Registered"}</td>
      <td>${entry.result ? `${entry.result.mark} ${entry.event.unit}` : "-"}</td>
      <td>${entry.rank || "-"}</td>
      <td><strong>${entry.points || 0}</strong></td>
    </tr>
  `).join("") || emptyRow(6, "No event entries found");
}
...
function meetFilePrefix() {
  return (state.meet.name || "meet").replace(/[^a-z0-9]+/gi, "_").replace(/^_+|_+$/g, "").toLowerCase() || "meet";
}
...
function printReport(title, sheets, mode) {
  if (!sheets.length) {
    toast(`No ${title.toLowerCase()} data available yet`, "error");
    return;
  }

  const html = reportHtml(title, sheets, mode);
  const win = window.open("", "_blank");
  if (!win) {
    downloadFile(`${meetFilePrefix()}_${title.replace(/[^a-z0-9]+/gi, "_").toLowerCase()}.html`, html, "text/html;charset=utf-8");
    toast("Popup blocked. Report downloaded as HTML instead.");
    return;
  }
  win.document.write(html);
  win.document.close();
  toast(`${title} opened for printing`);
}
...
function bindTabs() {
  $$(".nav-tabs button").forEach(button => {
    button.addEventListener("click", () => {
      if (button.dataset.organizerTab !== undefined && !state.organizerSession) {
        toast("Organizer login required", "error");
        return;
      }
      if (button.dataset.athleteTab !== undefined && !state.athletePortal) {
        toast("Athlete login required", "error");
        return;
      }
...
function bindForms() {
  ...
  $("#organizerLoginForm").addEventListener("submit", async event => {
    event.preventDefault();
    try {
      await api("/organizer-login", { method: "POST", body: JSON.stringify(formData(event.currentTarget)) });
      state.organizerSession = true;
      renderAccess();
      toast("Organizer desk unlocked");
      showTab("athletes");
    } catch (error) {
      toast(error.message, "error");
    }
  ...
  $("#dashboardAthleteLoginForm").addEventListener("submit", async event => {
    event.preventDefault();
    try {
      state.athletePortal = await api("/athlete-login", { method: "POST", body: JSON.stringify(formData(event.currentTarget)) });
      renderAthletePortal();
      renderAccess();
      showTab("athletePortal");
      toast("Athlete side opened");
    } catch (error) {
      state.athletePortal = null;
      renderAthletePortal();
      renderAccess();
      toast(error.message, "error");
    }
  ...
  $("#organizerRegisterForm").addEventListener("submit", async event => {
    event.preventDefault();
    try {
      const result = await api("/organizer-register", { method: "POST", body: JSON.stringify(formData(event.currentTarget)) });
      $("#organizerRegisterStatus").innerHTML = `
        <strong>${result.username}</strong> registered.<br>
        Use this username and password on the dashboard organizer login.
      `;
      event.currentTarget.password.value = "";
      toast(result.message || "Organizer registered");
    } catch (error) {
      $("#organizerRegisterStatus").textContent = error.message;
      toast(error.message, "error");
    }
  });

  $("#athleteRegisterForm").addEventListener("submit", async event => {
    event.preventDefault();
    try {
      const result = await api("/athlete-register", { method: "POST", body: JSON.stringify(formData(event.currentTarget)) });
      const credential = result.credential;
      $("#athleteRegisterStatus").innerHTML = `
        <strong>${credential.name}</strong><br>
        Bib: ${credential.bib}<br>
        Access Code: <strong>${credential.accessCode}</strong>
      `;
      toast(result.message || "Athlete credential ready");
    } catch (error) {
      $("#athleteRegisterStatus").textContent = error.message;
      toast(error.message, "error");
    }
  });
  ...
  $("#athleteLoginForm").addEventListener("submit", async event => {
    event.preventDefault();
    try {
      state.athletePortal = await api("/athlete-login", { method: "POST", body: JSON.stringify(formData(event.currentTarget)) });
      toast("Athlete view opened");
      renderAthletePortal();
      renderAccess();
    } catch (error) {
      state.athletePortal = null;
...
function bindActions() {
  ...
  $("#generateCredentialsBtn").addEventListener("click", async () => {
    try {
      const result = await api("/athlete-credentials", { method: "POST", body: JSON.stringify({}) });
  ...
  $("#downloadCredentialsBtn").addEventListener("click", () => {
    if (!state.athletes.length) {
      toast("No athletes available for credentials", "error");
      return;
    }
    downloadFile(`${meetFilePrefix()}_athlete_credentials.csv`, credentialsToCsv(state.athletes));
    toast("Athlete credentials downloaded");
  });
  ...
  $("#organizerLogoutBtn").addEventListener("click", () => {
    state.organizerSession = false;
    showTab("dashboard");
    renderAccess();
    toast("Organizer logged out");
  });
  ...
  $("#athleteLogoutBtn").addEventListener("click", () => {
    ...
    showTab("dashboard");
    renderAccess();
    toast("Athlete logged out");
  });

  $("#clearDataBtn").addEventListener("click", async () => {
    const confirmation = window.prompt("This will permanently clear athletes, events, entries, and results from the database. Type CLEAR to continue.");
    if (confirmation !== "CLEAR") {
      toast("Data clear cancelled");
      return;
    }

    try {
      const result = await api("/clear-data", { method: "POST", body: JSON.stringify({ confirm: confirmation }) });
      Object.assign(state, {
        meet: result.after?.meet || { name: "", venue: "", date: "" },
        athletes: [],
        events: [],
        registrations: [],
        results: [],
        ...
        dashboard: {
          meet: result.after?.meet || { name: "", venue: "", date: "" },
          totals: { athletes: 0, events: 0, registrations: 0, completedEvents: 0 },
          leaderboard: [],
          recentResults: []
        }
      });
      ...
      render();
      toast(result.message || "Meet data cleared");
      await loadAll();
    } catch (error) {
      toast(error.message, "error");
    }
  ...
  $("#downloadResultsBtn").addEventListener("click", () => {
    downloadFile(`${meetFilePrefix()}_results.csv`, toCsv(state.exportRows));
    toast("Results CSV downloaded");
  });

  $("#reloadReportsBtn").addEventListener("click", async () => {
    await loadAll();
    toast("Reports reloaded");
  });
  ...
  $("#downloadParticipationBtn").addEventListener("click", () => {
    ...
    toast("Participation report downloaded");
  });
}
